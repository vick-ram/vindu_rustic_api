...
import org.example.plugins.PesapalAuthException
import org.example.plugins.PesapalException
...
class PesapalClient(
    ...
    private val ipnUrl: String,
    private val callbackUrl: String,
    private val cancellationUrl: String
) {
    ...
    suspend fun getOrRegisterIpn(): String {
        // Return early if notification id already exists
        notificationId?.let { return it }

        val ipns = getRegisteredIpn()
        for (ipn in ipns) {
            if (ipn.url == ipnUrl) {
        ...
        // If not found, register new ipn
        val response = registerIpn(ipnUrl, "GET")
        notificationId = response.ipnId
        return response.ipnId
